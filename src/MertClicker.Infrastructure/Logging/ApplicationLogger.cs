...
namespace MertClicker.Infrastructure.Logging;
...
public sealed class ApplicationLogger : IApplicationLogger
{
    ...
    {
        _logger = loggerFactory.CreateLogger("MertClicker");
        _paths = paths;
    ...
    private void DeleteExpiredLogFiles()
    {
        ...
        try
        {
            filePaths = Directory.EnumerateFiles(_paths.LogsDirectory, "mertclicker-*.log").ToList();
        }
        catch
        {
        ...
        foreach (var filePath in filePaths)
        {
            try
            {
                ...
                var datePart = fileName["mertclicker-".Length..];
    ...
    private void AppendToFile(string level, Exception? exception, string message, object?[] args)
    {
        try
        {
            ...
            var filePath = Path.Combine(_paths.LogsDirectory, $"mertclicker-{DateTime.Now:yyyy-MM-dd}.log");
