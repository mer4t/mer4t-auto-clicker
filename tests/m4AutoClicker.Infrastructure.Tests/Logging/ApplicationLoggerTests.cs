...
public sealed class ApplicationLoggerTests : IDisposable
{
    ...
    public void Construction_Deletes_Log_Files_Older_Than_Retention_Period()
    {
        ...
        var oldFile = Path.Combine(paths.LogsDirectory, $"m4autoclicker-{DateTime.Now.AddDays(-30):yyyy-MM-dd}.log");
        var recentFile = Path.Combine(paths.LogsDirectory, $"m4autoclicker-{DateTime.Now.AddDays(-1):yyyy-MM-dd}.log");
    ...
    public void Construction_Deletes_Other_Expired_Files_Even_If_One_Is_Locked()
    {
        ...
        var lockedFile = Path.Combine(paths.LogsDirectory, $"m4autoclicker-{DateTime.Now.AddDays(-30):yyyy-MM-dd}.log");
        var deletableFile = Path.Combine(paths.LogsDirectory, $"m4autoclicker-{DateTime.Now.AddDays(-40):yyyy-MM-dd}.log");
