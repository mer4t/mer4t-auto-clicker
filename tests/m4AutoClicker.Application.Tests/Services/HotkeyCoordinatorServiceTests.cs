...
public class HotkeyCoordinatorServiceTests
{
    ...
    [Fact]
    public async Task ReassignHotkeys_When_New_Combination_Conflicts_Restores_Previous_Working_Hotkey()
    {
        // AutoClickerToggle F6'dan F8'e değiştirilmeye çalışılıyor, ama F8 aynı toplu kaydetme
        // içinde (değişmeyen) MacroPlaybackToggle tarafından önce alınıyor. Yeni kombinasyon
        // reddedilse bile kullanıcı AutoClickerToggle için elinde çalışan HİÇBİR kısayol
        // kalmamış olmamalı; önceki (F6) otomatik olarak geri yüklenmeli.
        var (coordinator, _, _, _, _, injector, hotkeyService, _, _) = CreateCoordinator();
        coordinator.RegisterDefaultHotkeys();

        var newBindings = new List<HotkeyDefinition>
        {
            new() { Id = HotkeyIds.MacroPlaybackToggle, Key = VirtualKey.F8 },
            new() { Id = HotkeyIds.AutoClickerToggle, Key = VirtualKey.F8 },
            new() { Id = HotkeyIds.MacroRecorderToggle, Key = VirtualKey.F7 },
            new() { Id = HotkeyIds.EmergencyStop, Key = VirtualKey.F9 }
        };
        ...
        // Dönen sonuç, kullanıcının bu kaydetmede GERÇEKTE ne istediğini ve neden reddedildiğini
        // dürüstçe yansıtmalı (arka planda otomatik geri yükleme yapıldı diye "başarılı"
        // görünmemeli) - aksi hâlde Ayarlar ekranı sessizce yanlış "Hazır" durumu gösterir.
        Assert.False(results[HotkeyIds.AutoClickerToggle].Success);
        Assert.Equal(HotkeyRegistrationErrorType.CombinationAlreadyUsed, results[HotkeyIds.AutoClickerToggle].ErrorType);

        // Ama arka planda önceki (F6) tanım otomatik olarak geri yüklenmiş olmalı: GetCurrentDefinition
        // ve GetRegistrationResult - diğer sayfaların ("Auto Clicker" durum metni gibi) ve bir
        // sonraki Ayarlar senkronizasyonunun doğru göreceği kalıcı durum - F6'nın gerçekten
        // çalıştığını göstermeli.
        var current = coordinator.GetCurrentDefinition(HotkeyIds.AutoClickerToggle);
        Assert.NotNull(current);
        Assert.Equal(VirtualKey.F6, current!.Key);
        Assert.True(coordinator.GetRegistrationResult(HotkeyIds.AutoClickerToggle)!.Success);

        coordinator.AutoClickerPlanProvider = () => FixedCountPlan();
        await coordinator.ToggleAutoClickerAsync();

        // F6 hâlâ gerçekten çalışıyor: eylem tetiklendi.
        Assert.NotEmpty(injector.PressedButtons);
    }
